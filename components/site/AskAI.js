            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 noise-hide">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm whitespace-pre-wrap ${m.role === 'user' ? 'bg-gradient-to-br from-primary to-accent text-primary-foreground' : 'bg-white/5 border border-white/10'}`}>{m.content}</div>
                </div>
              ))}
